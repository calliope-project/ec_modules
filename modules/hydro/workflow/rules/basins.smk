...
"""
# TODO: either configurable (should be in shema) or preset to always run.
...
rule hydrobasin_zip_extract:
    message: "Extract requested basin level (i.e., resolution)."
    input:
        zipfile = "results/basins/hydrobasin_{continent}.zip"
    output:
        shapefile = "results/basins/shape_{continent}.geojson",
        plot_shapefile = "results/plots/basin_{continent}.png"
    params:
...
rule preprocess_basins:
    ...
    input:
        basins = "results/basins/shape_eu.geojson"
    params:
    ...
    output: "results/basins/shape_preprocessed.gpkg"
